...
if($addEditStudentM=="YES" && $studentManagement=="YES")
{
 ...
 $addNewLink='<button type="button"  class="btn btn-sm btn-primary font-weight-bold py-2 px-3 px-xxl-5 my-1" style="margin-right:10px;" data-toggle="modal" data-target="#myModal">Add New House</button>';
 }
...
//output all from classes table
        ...
        $allSportsHouseResult = mysqli_query($con,  $allSportHouseQuery);
...

{
  ...
  $check = "SELECT * FROM sporthouse where houseName = '".$addNew."'";
  ...
  if( $addNew !== $data['houseName'] && $addNew!="")
  {
  $insertQuery = "INSERT INTO sporthouse (houseName) VALUES
  ('".$addNew."')";
  }
...
<script language="javascript" src="formjs/all-sport-house.js" type="text/javascript"></script>
<!--edit and delete form script-->
...
// side bar and header
    ...
    <main class="app-content">
      <div class="app-title">
      <div>
          <h1><i class="fa fa-th-list"></i> Sport Houses</h1>
          <p></p>
        </div>
            ...
            <div class="tile-body">
              ...
              <form name="frmUser" method="post" action="">
                <table class="table table-bordered  table-striped" id="sampleTable" style="width:60%; margin-left:18%">
                  ...
                  <tbody>
                   <?php
                   while ($allSportsHouseRow = mysqli_fetch_array($allSportsHouseResult))
                   {
                   //output all from students table
                    $house_id =$allSportsHouseRow['houseID']; 

                   $houseName =$allSportsHouseRow['houseName'];
                   ?>
                    <tr>
                      <td><input type="checkbox" name="users[]" value="<?php echo  $house_id; ?>" class="name" /></td>
                      <td><?php echo $allSportsHouseRow['houseID']; ?></td>
                      <td><?php echo $allSportsHouseRow['houseName'];?></a></td>
