...
$student_score=mysqli_query($con, "SELECT SUM(Average) AS Average, StudentID FROM `results` where class ='".$_SESSION['classResultClass']."' and school_session = '".$_SESSION['classResultSession']."' and Publish='Yes'  GROUP BY StudentID ");
...
//total average
        ...
        $Query = "SELECT StudentReg, SUM(Average) AS totalaverage FROM results WHERE Class ='".$_SESSION['classResultClass']."' AND school_session ='".$_SESSION['classResultSession']."'  AND Publish='Yes' AND StudentReg ='".$regNum."'
...
$current_class = substr($_SESSION['classResultClass'], 0, 5);
...
?></strong></p>
          ...
          <p>Commandant's Comment (<?php echo $principal?>): <?php echo $principal_promotion_comment;?></p>
...
?>
                ...
                <td class="column100 column3" data-column="column3"> <?php 
                  $check_res = $con->query("SELECT *, SUM(Average) AS Sum_Average FROM `results` WHERE school_session='$_SESSION[classResultSession]' AND class='$_SESSION[classResultClass]' AND  subjectID='$row_subjects[subjectID]' AND Publish ='YES'  GROUP BY StudentReg ORDER BY  Sum_Average DESC ");
                 // $score_ends = array(1 => "st", 2 => "nd", 3 => "rd", 4 => "th");
                  $counter = 1; 
                  $rank = 1; 
